...
*	  curerntly defined direction #defines don't allow for something
*	  nicer. Ptuh!
*/
int mhGetConnDir(const aMapHeader *mh, int tx, int ty)
{
...
void mapLoad(u32 index) //this one by Kawa
{
	...
	if((u32)currentMonsters[0] < 0x08000000) currentMonsters[0] = 0;
	if((u32)currentMonsters[1] < 0x08000000) currentMonsters[1] = 0;
	if((u32)currentMonsters[2] < 0x08000000) currentMonsters[2] = 0;
	if((u32)currentMonsters[3] < 0x08000000) currentMonsters[3] = 0;
	...
	NextWeatherEffect = 0;
	if(currentMapHeader->Weather && currentMapHeader->Weather < 7)
	{
		if(WeatherEffect)
		{
...
void camUpdate(int px, int py)
{
	...
	// Go to metatile size
	py -= 16;
	tpx= gCameraX>>4; tpy= gCameraY>>4;

	tcx= gCam.oldX>>4; tcy= gCam.oldY>>4;
	...
	if( tpx != tcx )
	{
		...
		for(ii=0; ii<16; ii++)
			mapDrawMeta(tmp+gMapOfsX, tpy+ii+gMapOfsY,
...
void HandleCamera()
{
	...
	{
		if(Actors[0].direction == 0) ny += WalkDistLUT[Actors[0].frameindex];
		if(Actors[0].direction == 1) nx += WalkDistLUT[Actors[0].frameindex];
		if(Actors[0].direction == 2) ny -= WalkDistLUT[Actors[0].frameindex];
		if(Actors[0].direction == 3) nx -= WalkDistLUT[Actors[0].frameindex];
	}
	if(Actors[0].state == ACTSTATE_HOP)
	{
		if(Actors[0].direction == 0) ny += hopOffsets[Actors[0].frameindex];
		if(Actors[0].direction == 1) nx += hopOffsets[Actors[0].frameindex];
		if(Actors[0].direction == 3) nx -= hopOffsets[Actors[0].frameindex];
	}
	camUpdate(nx, ny);
}
...
void FocusCam()
{
	REG_BG1HOFS= BGScrollX;	REG_BG1VOFS= BGScrollY;
	REG_BG2HOFS= BGScrollX;	REG_BG2VOFS= BGScrollY;
	REG_BG3HOFS= BGScrollX;	REG_BG3VOFS= BGScrollY;
}
