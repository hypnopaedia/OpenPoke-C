...
extern const unsigned short danibagTiles[4096];
extern const unsigned short danibagPal[16];
...
void DrawInventoryScreen(int page, int scrolloffset, int cursor)
{
	...
	{
		bagTiles = (short*)dannybagTiles;
		bagPal = (short*)dannybagPal;
	} else
	{
		bagTiles = (short*)danibagTiles;
		bagPal = (short*)danibagPal;
	}
...
/** \brief Writes the inventory list.
		\param pageoffset Which inventory page to start from
...
int PokeBall_GetSuccess(int ball)
{
	...
	int c = BaseStats[fighters[1].species].CatchRate;
	AGBPrintf("A = %d\nB = %d\nC = %d\n",a,b,c);
	switch(ball)
	{
		case 0: c = c * 255; break; //Master Ball - always works
		case 1: c = c * 2; break; //Ultra Ball - 2x
		case 2: c = c + (c/2); break; //Great Ball - 1.5x
		case 3: c = c * 1; break; //Poké Ball - 1x
		case 4: c = c + (c/2); break; //Safari Ball - 1.5x
		case 5: c = c * 3; break; //Net Ball - 3x if Water/Bug
		case 6: c = c * 4; break; //Dive Ball - 4x if underwater
		case 7: c = c * 1; break; //Nest Ball - (40-(wild Pokemon's level))/10 if level < 30
		case 8: c = c * 3; break; //Repeat Ball - 3x if caught before
		case 9: c = c * 1; break; //Timer Ball - ((No. of turns)+10)/10 top off at 4x
		case 10: c = c * 1; break; //Luxury Ball - 1x
		case 11: c = c * 1; break; //Premier Ball - 1x
	}
	AGBPrintf("C+ = %d\n", c);

	int x1 = ((a*3-b*2)*c)/(a*3);
	int x2 = x1;
	AGBPrintf("X = %d\n",x1);
	if(fighters[1].mon->Status.AilmentSleep) x2 = x1 * 2;
	else if(fighters[1].mon->Status.AilmentFrozen) x2= x1 * 2;
	else if(fighters[1].mon->Status.AilmentBurned) x2 = x1 + (x1/2);
	else if(fighters[1].mon->Status.AilmentParalysis) x2 = x1 + (x1/2);
	else if(fighters[1].mon->Status.AilmentPoison) x2 = x1 + (x1/2);
	AGBPrintf("X+ = %d\n",x2);
	if(x2 > 255) return 0;
...
int item_PokeBall(u16 index, u16 special1, u16 special2, u32 param1, u32 param2)
{
	...
	if(i==0)
	{
		...
		if(!GETBIT(MyDex.caught, fighters[1].species))
		{
			...
			Register(fighters[1].species, REG_CAUGHT | REG_SHOWDEX);
		...
		if(ps < 6)
		{
			DmaCopy(&HisParty[0], &MyParty[ps], sizeof(aPokemon)); //assuming wild monster is always #0...
			mon = &MyParty[ps];
		} else
		{
...
int item_BtlHeal(u16 index, u16 special1, u16 special2, u32 param1, u32 param2)
{
	...
	if(pk != -1)
	{
		...
		{
			int amount = clamp(MyParty[pk].Status.CurrentHP + special2, 0,
												 MyParty[pk].Status.TotalHP - MyParty[pk].Status.CurrentHP + 2);
			int oldAmount = amount;
			if(amount == 0)
			{
				sayf(SYSTEM, strItemHasNoEffect);
...
int item_Candy(u16 index, u16 special1, u16 special2, u32 param1, u32 param2)
{
	...
	if(pk != -1)
	{
		...
		//Check for moves
		const aLearnedMove * moveset = MoveSets[GetMonsterStats(&MyParty[pk], pSpecies)];
		int l;
		for(l = 0; l < 100; l++)
		{
