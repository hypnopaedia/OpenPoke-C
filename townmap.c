...
void TownMap()
{
	...
	for(i = 0; ; i++)
	{
		if(MapLocations[i].x == 0xFF) break;
		MapHotspots[MapLocations[i].x][MapLocations[i].y][!(i<20)] = i;
		if(MapLocations[i].shape == 1 || MapLocations[i].shape == 3) MapHotspots[MapLocations[i].x][MapLocations[i].y+1][0] = i;
		if(MapLocations[i].shape == 2 || MapLocations[i].shape == 3) MapHotspots[MapLocations[i].x+1][MapLocations[i].y][0] = i;
		if(MapLocations[i].shape == 3) MapHotspots[MapLocations[i].x+1][MapLocations[i].y+1][0] = i;
	}
...
//REG_DISPCNT =  DCNT_MODE0 | DCNT_BG0 | DCNT_WIN0;
	...
	while(1)
	{
		...
		KeyRead();
		if(Trg & KEY_B) break;
		if(Cont & KEY_UP && !cursorDelay && cursorY > 3) cursorY--;
		if(Cont & KEY_DOWN && !cursorDelay && cursorY < 17) cursorY++;
		if(Cont & KEY_LEFT && !cursorDelay && cursorX > 4) cursorX--;
		if(Cont & KEY_RIGHT && !cursorDelay && cursorX < 25) cursorX++;
		if(Cont && !cursorDelay) cursorDelay = 5;
		if(Cont)
		{
			REG_WIN0V = 0;
			vwClear(0);
