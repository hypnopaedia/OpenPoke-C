...
int GetPartySize(aPokemon * party)
{
	int i;
	for(i=0; i<7; i++)
		if(GetMonsterStats(&party[i], pSpecies) == 0) return i;
...
int GetDexNumber(int national)
{
	...
	{
		if(NationalDex[ii] == national) return ii;
	}
	return national;
}
...
int MonHasAbility(aPokemon * mon, int ability)
{
	...
	if(mon->Personal.Personal % 2)	a = BaseStats[species].Ability2;
	else														a = BaseStats[species].Ability1;
	if(a==0)												a = BaseStats[species].Ability1;
	if(a == ability) return 1;
	return 0;
}
