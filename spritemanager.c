...
void FindAndRunScript()
{
	...
	{
		sayf(SYSTEM,SimpleTileMessages[(char)thisFlags]);
		return;
	}
	switch(thisFlags)
	{
...
//		}
	...
	for(i = 0; i < nac; i++)
	{
		...
		if(actor->sprite)
		{
			...
			infrontx = actor->mx;
			infronty = actor->my;
			if(actor->direction == ACTFACING_SOUTH) infronty++;
			if(actor->direction == ACTFACING_WEST)  infrontx++;
			if(actor->direction == ACTFACING_NORTH) infronty--;
			if(actor->direction == ACTFACING_EAST)  infrontx--;
...
*/
			...
			if(Actors[i].state == ACTSTATE_TURN)
			{
				if(Actors[i].timer == 0)
				{
					Actors[i].timer= Actors[i].speed;
			...
			if((Actors[i].state == ACTSTATE_WAVE) || (Actors[i].state == ACTSTATE_JOYNOD))
			{
				Actors[i].timer++;
			...
			if(Actors[i].state == ACTSTATE_PING)
			{
			...
			if(Actors[i].state == ACTSTATE_HOP)
			{
				if(Actors[i].timer == 0)
				{
			...
			if(Actors[i].state == ACTSTATE_SURFUP)
			{
				if(Actors[i].timer == 0)
				{
					Actors[i].timer = Actors[i].speed;
			...
			} else if(Actors[i].state == ACTSTATE_SURFTURN)
			{
				if(Actors[i].timer == 0)
				{
					Actors[i].timer= Actors[i].speed;
