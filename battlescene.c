...
void scene_DrawMoveData(aPokemon * mon, int move)
{
	...
	int m = GetMonsterStats(mon, pMove1 + move);
	int p = GetMonsterStats(mon, pPP1 + move);
	vwWriteG(16,8,TypeNames[moves[m].type]);
	sprintf(ppline,"PP: %d/%d", p, GetPPForMove(mon, move)); //moves[move].pp);
...
void scene_StartTrainerBattle(int index)
{
	...
	for(i = 0; i < 64; i++)
	{
		...
		if(i < 48) OamBak[0].CharNo = 512 + (64 * (i/10));
	}
//	OamBak[0].CharNo = 0;
//	OamBak[0].Pltt = 0;
//	OamBak[0].HPos = 24;
...
// </demo>
	//Runaway hack
	if(demo==0 && Cont & KEY_A && Cont & KEY_B)
	{
...
void scene_WriteInBox(u32 where, char *what)
{
	...
	switch(where)
	{
		case 0: x=148; y=13; break;
		case 1: x=148; y=-3; break;
		case 2: x=200; y=22; break;
	}
...
int scene_CommandMenu(int index)
{
	char buffy[255];
	...
	while(1)
	{
		...
		if((Trg & KEY_LEFT) || (Trg & KEY_RIGHT))
		{
			...
			if(lastaction == 3 && ThisBattle.trainer) lastaction = poop;
			scene_SetLine(actioncursors[lastaction*2],1,actioncursors[(lastaction*2)+1],255,15);
		}
...
int scene_FightMenu(int index)
{
	...
	int lastmove = fighters[index].lastused;
	...
	vwWriteG(0,8, moveNames[GetMonsterStats(fighters[index].mon, pMove1)]);
	vwWriteG(0,10,moveNames[GetMonsterStats(fighters[index].mon, pMove2)]);
	vwWriteG(0,12,moveNames[GetMonsterStats(fighters[index].mon, pMove3)]);
	vwWriteG(0,14,moveNames[GetMonsterStats(fighters[index].mon, pMove4)]);
	vw_SetBig();
	scene_SetLine(movecursors[lastmove*2],1,movecursors[(lastmove*2)+1],255,15);
	scene_DrawMoveData(fighters[index].mon, lastmove);
