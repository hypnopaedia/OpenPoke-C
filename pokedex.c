...
void PokeDex_ShowData(int idx, int page, int flavorpage)
{
	...
	} else
	{
		...
		for(meh = 0; meh < 5; meh++)
		{
			...
			{
				case evoLevel:
					sprintf(foo, strPokedexEvolvesLevel, (char*)GetPokemonName(Evolutions[(idx*4*5)+(meh*4)+2]), Evolutions[(idx*4*5)+(meh*4)+1]);
					strcat(data, foo);
					break;
				case evoHappy:
...
*/
void Register(int species, int flags)
{
	...
	AGBPrintf("Register: species #%d is now marked as seen.\n", species);
	...
	if(flags & REG_CAUGHT)
	{
		...
		AGBPrintf("Register: species #%d is now marked as caught.\n", species);
	}
