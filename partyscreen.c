...
void StatusText(aPokemon * toshow, u32 page, u32 oldpage)
{
	char buff[256];
	int i;
	int isEgg = GetMonsterStats(toshow, pEgg);

	int lvl = GetMonsterStats(toshow, pLevel);
	...
	if(page==0)
	{
		...
		vwWrite(112,6, buff);
		if(toshow->Personal.Font == 1) vw_SetJapanese();
		vwWrite(112,21, toshow->Personal.Nickname);
		vwWrite(112,39, toshow->Personal.OTName);
		if(toshow->Personal.Font == 1) vw_SetBig();
		sprintf(buff,"%d", (u16)GetMonsterStats(toshow, pOTID));
		vwWrite(112,54, buff);
		int hi = GetMonsterStats(toshow, pHeldItem);
		if(hi)
			vwWrite(0,40, items[hi].name);
		else
			vwWrite(0,40, "None");
		sprintf(buff, strStatusNature, natures[GetMonsterStats(toshow, pPersonal) % 25]);
		vwWrite(4,81, buff);

		int otid = GetMonsterStats(toshow, pOTID);
		int loc = GetMonsterStats(toshow, pLocationMet);
		int lvlmet = GetMonsterStats(toshow, pLevelMet);
		...
		for(i = 0; i < 6; i++)
		{
			if(GetMonsterStats(toshow, i + pDVHP) > highestStatVal)
			{
				highestStat = i;
				highestStatVal = GetMonsterStats(toshow, i + pDVHP);
			}
		}
	...
	{
		sprintf(buff,"%3d/%3d", toshow->Status.CurrentHP, toshow->Status.TotalHP);
		vwWrite(128,5,buff);
		sprintf(buff,"%d", toshow->Status.Attack);
		vwWrite(144,19,buff);
		sprintf(buff,"%d", toshow->Status.Defense);
		vwWrite(144,33,buff);
		sprintf(buff,"%d", toshow->Status.Speed);
		vwWrite(144,47,buff);
		sprintf(buff,"%d", toshow->Status.SpAttack);
		vwWrite(144,61,buff);
		sprintf(buff,"%d", toshow->Status.SpDefense);
		vwWrite(144,75,buff);
		sprintf(buff,strStatusExpToNext, GetMonsterStats(toshow,pExperience), XPRequirements[(BaseStats[species].LevelSlope * 101) + lvl + 1] - GetMonsterStats(toshow,pExperience));
		vwWrite(0,42,buff);
	...
	} else if(page==2)
	{
		...
		for(i=0; i<4; i++)
		{
			if(GetMonsterStats(toshow, pMove1 + i))
			{
				sprintf(buff,"%d/%d", GetMonsterStats(toshow, pPP1 + i), GetPPForMove(toshow, i)); //moves[myA->Attack[i]].pp);
	...
	{
		//TODO	Egg status
		int t = GetMonsterStats(toshow, pTameness);
		if(t <= 5)
			i = 3;
		else if(t <= 10)
			i = 2;
...
extern const unsigned short statusPal[16];
extern const unsigned short statusTiles[4752];
extern const unsigned short statusMap[9216];
...
void ShowStatus(aPokemon * toshow)
{
	...
	DmaArrayCopy(statusPal, MEM_PAL_BG);
	DmaArrayCopy(statusTiles, MEM_VRAM + 0x8000);
	DmaCopy(statusMap, MEM_VRAM + 0x4800, 10240);
	ClearOamBak();

	DmaArrayCopy(markingsPal, MEM_PAL_OBJ + 0x20);
	DmaCopy(markingsTiles, MEM_VRAM_OBJ + 0x1000, 128);
	if(toshow->Personal.Marking & 1) DmaCopy(markingsTiles+64,  MEM_VRAM_OBJ + 0x1000,    32);
	if(toshow->Personal.Marking & 2) DmaCopy(markingsTiles+80,  MEM_VRAM_OBJ + 0x1000+32, 32);
	if(toshow->Personal.Marking & 4) DmaCopy(markingsTiles+96,  MEM_VRAM_OBJ + 0x1000+64, 32);
	if(toshow->Personal.Marking & 8) DmaCopy(markingsTiles+112, MEM_VRAM_OBJ + 0x1000+96, 32);

	species = GetMonsterStats(toshow, pPokeBall);
	...
	species = GetMonsterStats(toshow, pSpecies);

	if(GetMonsterStats(toshow, pEgg))
	{
		LoadPokemonPic(pkEgg, 0,MEM_VRAM_OBJ,pal_obj_bank[0],0);
		DmaCopy(statusMap + 7168, MEM_VRAM + 0x5000, 2048);
		DmaClear(0, MEM_VRAM_OBJ + 0x1000, 0x80);
		page = 4;
		oldpage = 4;
	} else
	{
		LoadPokemonPic(species, (LPP_SHINY*IsShiny(toshow)),MEM_VRAM_OBJ,pal_obj_bank[0],toshow->Personal.Personal);
	}
	...
	StatusText(toshow, page, oldpage);
	...
	while(1)
	{
		DoVBlank();
		KeyRead();
		if(Trg & START_BUTTON) WriteCompleteDocs(toshow);
		if(Trg & B_BUTTON)
		{
			break;
		}
		if((Trg & L_KEY) || (Trg & R_KEY))
		{
			...
			if(oldpage != page)
			{
				...
				} else if(page == 1) //Skills
				{
					...
					DmaCopy(statusMap + 1024, MEM_VRAM + 0x5000, 2048);
				} else if(page == 2) //Moves
				{
					REG_BG1CNT = BG_4BPP | BG_SIZE1 | BG_PRIO(2) | 12 << BG_SBB_SHIFT | 2 << BG_CBB_SHIFT;
					REG_BG2CNT = BG_4BPP | BG_SIZE1 | BG_PRIO(1) | 10 << BG_SBB_SHIFT | 2 << BG_CBB_SHIFT;
					DmaCopy(statusMap + 5120, MEM_VRAM + 0x5000, 2048);
					FlipCards(1);
				}
				StatusText(toshow, page, oldpage);
				oldpage = page;
...
/** \brief Displays the party and lets the player select a monster to somehow use depending on purpose. */
int ShowParty(int purpose, int additional)
{
	...
	while(1)
	{
		...
		if((Trg & L_KEY) || (Trg & R_KEY))
		{
			...
			sel = wrap(sel + key_tri_horz(), 0, ps);
			CopyPartyBoxPal(sel, 1);
		...
		if((Trg & U_KEY) || (Trg & D_KEY))
		{
			CopyPartyBoxPal(sel, 0);
			OamBak[sel+10].CharNo = 258;
			sel = wrap(sel + key_tri_vert(), 0, ps);
			CopyPartyBoxPal(sel, 1);
			OamBak[sel+10].CharNo = 274;
		}
		if(Trg & A_BUTTON)
		{
			...
			} else
			{
				...
				{
					int j = MultipleChoice(18,12,12, 19,1); //ChoicePartyItems);
					if(j == 0) //Give
					{
						FadeOut();
						j = HoldInventory();
...
int SelectLearnMove(aPokemon *target, int mid)
{
	...
	DmaCopy(statusMap + 5120, MEM_VRAM + 0x5000, 4000);
	OamBak[8].CharNo = 256;
	OamBak[8].Shape = 1;
	OamBak[8].Size = 3;
	OamBak[8].HPos = 117;
