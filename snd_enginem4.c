...
{
	if(snd_TrackNowPlaying == track) return;
...
*/
	...
	while(1)
	{
		...
		{
			selectedTrack = wrap(selectedTrack + key_tri_horz(), 0, 85);
			vwClearLine(1,0);
			vwClearLine(1,1);
			vwWrite(0,0,"\x80\x1A\x80\x1B");
			sprintf(buf, "%d. %s",selectedTrack,songNames[selectedTrack]);
...
/*
		for(i = 0; i < MAX_MUSICPLAYER_TRACK; i++)
		{
			t = (MusicPlayerTrack*)&mplay_table[0].ta[i];
			if(t->d[0] == 0x80)
				vwWrite((i*4)+80,32,"|");
			else
				vwWrite((i*4)+80,32,".");
		...
		vol = 0;
		numchans = 0;
		for(i = 0; i < MAX_DIRECTSOUND_CHANNEL; i++)
		{
			if(m4a_sound.vchn[i].sf)
			{
				vwWrite((i*4)+80,48,"|");
				vol += m4a_sound.vchn[i].rv + m4a_sound.vchn[i].lv;
				numchans++;
			} else
				vwWrite((i*4)+80,48,".");
		}
		sprintf(buf,"%d     ",vol / numchans);
		vwWrite(80,64,buf);
*/
	}
}
