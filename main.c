...
void CheckNoSave()
{
	...
	{
		sayf(SYSTEM,"This test requires a savegame.\r\nChoose “Normal Boot” and save\nyour game first.");
		SoftReset();
	}
}
#endif
...
int main(void)
{
	...
	*(vu32 *)INTR_VECTOR_BUF = (vu32 )intr_main;
	...
	AGBPrintf("Startup finished. Please check 0x2010000 in\nMemViewer for realtime player info.\n\n");
	KeyRead();
	...
	//Register(pkCharmander, REG_CAUGHT);
	//Register(pkSquirtle, 0);
	//Register(pkCaterpie, 0);
	//Register(pkWeedle, 0);
	//Register(pkSandshrew, 0);
	//PokeDex();
	...
	LoadFrame();
	KeyRead();
	if(Cont & KEY_START) TestTileset();
	if(Cont & KEY_SELECT) sndTest();
	foo = MultipleChoice(0,0,17, 21,0);
	switch(foo)
	{
		case 0:
			CheckNoSave();
		...
		case 2:
			CheckNoSave();
			CreateMonster(&MyParty[0],pkBulbasaur,5);
			CreateMonster(&HisParty[0],pkLickitung,5);
		...
		case 3:
			...
			sndPlaySound(4,SOUND_NORESTART);
			Battle(1,14,0,(void*)"Great Scott!");
			break;
		case 4:
			CheckNoSave();
			StartDemo(1);
...
*/
	...
	while(1)
	{
		...
		if(TweenProcess != 0) TweenProcess();
		if(CurrentProcess != 0) CurrentProcess();
		if(Trg & START_BUTTON && CurrentProcess==0 && (Actors[0].state == ACTSTATE_IDLE || Actors[0].state == ACTSTATE_SURF)) DoStartMenu();
		if(Trg & A_BUTTON)
		{
			if(CurrentProcess == 0)
				FindAndRunScript();
		...
		if(Trg & SELECT_BUTTON) UseRegisteredItem();
/*
		{
...
void TweenTimeAdjustedColors()
{
	...
	if(currentLevel->tilesetb) src = currentLevel->tilesetb->colordata;
...
int MultipleChoice(int left, int top, int width, int choices, int bcancel) //CHOICELIST choices)
{
	...
	while(1)
	{
		...
		{
			c = set->NumChoices - 1; //choices.numchoices - 1;
			break;
		}
...
void sayf(int style, const char *fmt, ...)
{
	...
	if(!SM_Dialoguebox.OneUp) Script_WAITKEYPRESS(0);
}
...
void ShowPics()
{
	...
	while(1)
	{
		...
		tid = wrap(tid + key_tri_horz(), 0, tpTotal); //tpLAST);
		if(tid != otid)
		{
		...
		pid = wrap(pid - key_tri_vert(), 1, pkCount-2);
		if(pid != opid)
		{
			reload = 1;
			vwClearLine(1,4);
