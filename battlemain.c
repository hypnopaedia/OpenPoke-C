...
//	ThisBattle.background = battlebacklist[MyPlayer.currentMap]; //background;
	...
	if(kind == 1) ThisBattle.trainer = 1;
...
void battle_OpenAnimation()
{
	...
	for(i = 0; i < 128; i++)
	{
		...
		OamBak[1].HPos = 256 + 151 + BattlePanOffset;
		DoVBlank();
	}
	BattlePan(1);
}
...
void battle_SendOut(int side, int partyindex, int flags)
{
	...
	if(side==0)	fighters[side].mon = &MyParty[partyindex];
	else				fighters[side].mon = &HisParty[partyindex];
...
//	}
	...
	if(side == 0) //you
	{
		...
		OamBak[0].HPos = 39;
		OamBak[0].VPos = 48 + fighters[side].spriteVOffset; //54;
		OamBak[0].Priority = 2;
...
//		}
	...
	} else
	{
		...
		OamBak[1].HPos = 144;
		OamBak[1].VPos = 8 + fighters[1].spriteVOffset; //16;
		OamBak[1].Priority = 2;
...
const char * battle_ThisEx(int fighterindex, int monsterindex)
{
	...
	for(i = 0; i < 10; i++)
	{
		if(party[monsterindex].Personal.Nickname[i] == 0) break;
		namebuf[i] = party[monsterindex].Personal.Nickname[i];
	}
	namebuf[i] = 0;
	if(battle_IsOpposing(fighterindex))
	{
		if(ThisBattle.trainer)
		{
			sprintf(__pbThis[monsterindex],"Foe %s",namebuf);
		} else
		{
			sprintf(__pbThis[monsterindex],"Wild %s",namebuf);
		}
	} else
	{
...
int battle_MaxLevel(aPokemon * party)
{
	...
	for(i=0; i < 6; i++)
	{
		...
		{
			if(party[i].Status.Level > lv) lv = party[i].Status.Level;
		} else
		{
			break;
		}
...
void battle_AutoChooseMove(int idxPokemon)
{
	...
	} else
	{
		...
		choices[idxPokemon][3] = idxPokemon ^ 1; //single-battle hack, should register as "the opponent". //-1;
	}
}
...
{
	int i,j;
	for(i = 0; i < 4; i++) //update participants in battles (unfainted participants will earn EXP even if they faint in between)
	{
...
{
	if(!battle_CanShowCommands(idxPokemon)) return false;
...
int battle_CanSwitch(int idxPokemon, int idxTo, int showMessages)
{
	...
	if(fighters[opp1].ability == abilityShadowTag) opp = opp1;
	if(fighters[opp2].ability == abilityShadowTag) opp = opp2;
	if(opp > -1)
	{
		if(showMessages)
		{
...
*/
							...
							if(newlevel > curlevel)
							{
								...
								//Check for moves
								const aLearnedMove * moveset = (aLearnedMove*)&MoveSets[species];
								int l;
								for(l = 0; l < 100; l++)
								{
									...
									if((moveset[l].Level <= newlevel) && (moveset[l].Level > curlevel))
									{
										...
										scene_BuildPlayerWin();
										scene_BuildOpponentWin();
										if(fighters[0].mon->Status.CurrentHP)	battle_SendOut(0, fighters[0].pokemonIndex, 0);
										if(fighters[1].mon->Status.CurrentHP)	battle_SendOut(1, fighters[1].pokemonIndex, 0);
										OamBak[2].HPos = 158;
										OamBak[3].HPos = 190;
										OamBak[4].HPos = 36;
										OamBak[5].HPos = 68;
...
int battle_CanChooseNonActive(int index)
{
	...
	for(i=0; i <= ps; i++)
	{
		if(battle_CanSwitchLax(index, i, 0)) return true;
	}
	return false;
}
...
void battle_Switch()
{
	...
	for(i=0; i<4; i++)
	{
		if((!ThisBattle.doublebattle) && (battle_IsDoubleBattler(priority[i]))) continue;
		if(fighters[priority[i]].mon->Status.CurrentHP) continue;
...
void battle_CommandPhase()
{
	...
	int cmd, index, pkmn = 0, item = 0, run;
	int target = 0;
	...
	for(i=0; i<4; i++)
	{
		if((!ThisBattle.doublebattle) && (battle_IsDoubleBattler(i))) continue;
		if((battle_IsOpposing(i)) && (fighters[fighters_Opposing1(i)].effTimeStop)) continue;
		if(battle_IsOpposing(i))
		{
		...
		} else
		{
			...
			if(battle_CanShowCommands(i))
			{
				while(1)
				{
					cmd = scene_CommandMenu(i);
					if(cmd == COMMAND_FIGHT)
					{
						if(battle_CanShowFightMenu(i))
						{
							if(battle_AutoFightMenu(i)) commandDone=1;
							while(1)
							{
								index = scene_FightMenu(i);
								if(index<0) break;
								if(!battle_RegisterMove(i,index)) continue;
...
{
	if(fighters[i].mon->Status.CurrentHP <= 0) return false;
...
void battle_AttackPhase()
{
	...
	for(i=0; i<4; i++)
	{
		if(choices[i][0]==CHOICE_SWITCH)
		{
			...
			for(j = 3; j >= 0; j--)
			{
				if(!fighters_IsOpposing(i,j)) continue;
				...
				if(fighters[i].mon->Status.CurrentHP <= 0) break;
			}
			if(battle_IsOpposing(i))
			{
